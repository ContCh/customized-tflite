...
void TfLiteSerializer::ExportToTfLite(const Model& model, std::string output_path) {
    ...
    auto tflite_model =
        CreateModel(builder, TFLITE_SCHEMA_VERSION, op_codes, subgraphs, description,
...
TfLiteSerializer::ExportSubGraphs(const Model& model, flatbuffers::FlatBufferBuilder* builder) {
...
TfLiteSerializer::ExportOpCodes(const Model& model, flatbuffers::FlatBufferBuilder* builder) {
    ...
    common::transform(op_type_table_, std::back_inserter(op_codes_vec), [&](OperatorType op_type) {
        ...
        int8_t deprecated_builtin_code = std::min(
            static_cast<int32_t>(builtin_op_code),
        ...
        return tflite::CreateOperatorCode(*builder, deprecated_builtin_code, custom_code, version,
                                          builtin_op_code);
    });
    return builder->CreateVector(op_codes_vec);
}

std::vector<Offset<tflite::Buffer>>
TfLiteSerializer::ExportBuffers(const Model& model, flatbuffers::FlatBufferBuilder* builder) {
...
Offset<Vector<Offset<tflite::Tensor>>>
TfLiteSerializer::ExportTensors(const Graph& subgraph, flatbuffers::FlatBufferBuilder* builder) {
    auto data_blobs = subgraph.GetDataBlobs();
    std::sort(data_blobs.begin(), data_blobs.end(),
              [](const DataBlob* blob1, const DataBlob* blob2) {
                  return blob1->GetName() < blob2->GetName();
              });
    ...
    for (const auto* data_blob : data_blobs) {
        Offset<tflite::QuantizationParameters> quant_param =
        ...
        if (data_blob->HasQuantParam()) {
            ...
            quant_param =
                tflite::CreateQuantizationParameters(*builder, min, max, scale, zero_point);
        }

        auto tensor_type = utils::GetMappedDataTypeOf(data_blob->GetDataType());
        auto shape       = data_blob->GetShape().GetDims();
        auto tensor =
            tflite::CreateTensor(*builder, builder->CreateVector(shape), tensor_type,
...
TfLiteSerializer::ExportOperators(const Graph& subgraph, flatbuffers::FlatBufferBuilder* builder) {
    ...
    for (auto* op : ops) {
        ...
        common::transform(op->GetOutputs(), std::back_inserter(outputs), [&](int32_t out_blob_id) {
            return data_blob_index_map_.at(out_blob_id);
        });
        ...
        tflite_ops.push_back(tflite::CreateOperator(
            *builder, op_index, builder->CreateVector(inputs), builder->CreateVector(outputs),
            option_type, op_option, 0, ::tflite::CustomOptionsFormat_FLEXBUFFERS));
    }
