...
void getCurrentTime(char* time_buf) {
    ...
    sprintf(time_buf, "%04d-%02d-%02d %02d:%02d:%02d.%03d", rough_time->tm_year + 1900, rough_time->tm_mon + 1,
            rough_time->tm_mday, rough_time->tm_hour, rough_time->tm_min, rough_time->tm_sec,
...
void LogSettings::SetTargetLogFile(const std::string& file_path) {
    ...
    if (FILE* fp = fopen(output_destination_.c_str(), "w")) {
        ...
        printf(
            "The file path not initialize, target path `%s` not exist. "
...
LogMessage::LogMessage(uint32_t level, const char* file, int line) : level_(level) { initLogStream(level, file, line); }
